...
class Parse {
    ...
    parseSingleInput (copy, input, prevPositions, color) {
        // const newCostumeSVG = original.originalCostume;
        // const copyOfCostume = {};
        // Object.assign(copyOfCostume, newCostumeSVG);
        if (prevPositions.length > 0) {
        ...
        if (toSplit.includes('L')) {
            ...
            const fullString = filteredString;
            const letter = filteredString.splice(0, 1);
            const theColor = letter[0].slice(2);
            color = theColor;
            // eslint-disable-next-line no-console
            console.log(color, 'what color do i have');
            const positions = this.convertBase(filteredString[0]);
            // eslint-disable-next-line no-console
            console.log(positions, 'pos');
            const absolute = positions.map(pos => +pos + Cast.toNumber(1));
            absolute.map(item => copy[`Light${item}`] = `"#${color}"`);
            absolute.unshift(Cast.toString(color));
            prevPositions.push(absolute.toString());
        }
